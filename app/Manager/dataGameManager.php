...
class dataGameManager
{
    ...
    {
        $sql = 'SELECT species.name,type_building.name , count(*) as nb_building, level*5 as max_quantity FROM building, type_building, species WHERE id_type = type_building.id and id_species = species.id and building.id_user LIKE :id group by id_type ';
        $stmt = $pdo->prepare($sql);
        $stmt->bindValue(':id', $id);
        $stmt->execute();
...
/*    /**
    ...
    public function getUserProductsAnimalsInformations($pdo, $id)
    {
        ...
        $sql = 'SELECT s2.quantity, vc.name FROM `stocks2` s2 JOIN users u ON (s2.id_users=u.id), variety_cereals vc where s2.stock_type=\'field\' AND s2.`id_product`=vc.id GROUP BY vc.id UNION SELECT s2.quantity, pa.name FROM `stocks2` s2 JOIN  users u ON (s2.id_users=u.id), product_animal pa where s2.stock_type=\'animal\' AND s2.`id_product`=pa.id GROUP BY pa.id';
