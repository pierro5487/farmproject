...
class dataGameManager
{
    ...
    public function getUserProductsAnimalsInformations($pdo, $id)
    {
        ...
        $sql = 'SELECT s.quantity, vc.name FROM `stocks` s JOIN users u ON (s.id_users=:id), variety_cereals vc where s.stock_type=\'field\' AND s.`id_product`=vc.id GROUP BY vc.id UNION SELECT s.quantity, pa.name FROM `stocks` s JOIN  users u ON (s.id_users=:id), product_animal pa where s.stock_type=\'animal\' AND s.`id_product`=pa.id GROUP BY pa.id';
